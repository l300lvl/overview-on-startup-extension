...
//        overlaySwitch.connect('notify::active', Lang.bind(this, this._setOverViewMode));
        ...
        let introLabel = _("Show Applications View");
        let radio = null;
        ...
        //build radio buttons
        let currentPosition = this._settings.get_string(Keys.APPDISPLAY);
        let count = 2;
        let str = '';
        for (element in RADIO_BTNS) {
            ...
            radio.connect('toggled', Lang.bind(this, function(widget) {
                if (widget.active)
                    this._settings.set_string(Keys.APPDISPLAY, str);
                }));
...
function buildPrefsWidget() {
    let widget = new ShowSettingsWidget();
